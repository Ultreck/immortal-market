...
export const getResizeHandles = ({ axis, ref, active }) => {
  if (['se', 'sw', 'ne', 'nw'].includes(axis)) {
    return (
      <div
        ref={ref}
        className={cn('absolute rounded-full bg-white shadow border border-gray-400 z-[2] w-[14px] h-[14px]', {
  ...
  if (['n', 's', 'e', 'w'].includes(axis)) {
    return (
      <div
        ref={ref}
        className={cn('absolute rounded-full bg-white shadow border border-gray-400 z-[2]', {
          ...
          '-right-[4px] top-1/2 -translate-y-1/2 w-[8px] h-[28px] cursor-e-resize': axis === 'e',
          hidden: !active,
        })}
      />
    );
