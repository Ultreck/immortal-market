...
const Table9 = ({ element, active, highlighted, width, onClick, onChange }) => {
  return (
    <ElementWrapper
      element={element}
      onClick={onClick}
...
Table9.propTypes = ElementPropTypes;
  ...
  return (
    ...
    >
          ...
          {headers.map((_, index) => {
            ...
            return (
              <th
                ...
                style={{ ...style, background: index % 4 < 2 ? colors[0] : colors[1] }}
              >
                {_}
              </th>
            );
      ...
      <tbody style={{ background: colors[2] }}>
        {rows.map((row, index) => (
          <tr key={`row-${index}`} style={{ background: index % 2 !== 0 ? colors[3] : '' }}>
            {row.map((_, index) => {
              ...
              return (
                <td key={`cell-${index}`} style={{ ...style }} className="border border-gray-300 h-fit">
                  {_}
                </td>
              );
            })}
