...
const Table10 = ({ element, active, highlighted, width, onClick, onChange }) => {
  return (
    <ElementWrapper
      element={element}
      onClick={onClick}
...
Table10.propTypes = ElementPropTypes;
              ...
              return (
                <td key={`cell-${index}`} className="border border-gray-300 h-fit" style={{ background: colors[2] }}>
                  <div className="flex flex-col h-full">
                    {cell.map((_, index) => (
                      <div key={`data-${index}`} className="border-b h-1/2 px-2 flex flex-col justify-center">
                        {_}
                      </div>
                    ))}
                  </div>
