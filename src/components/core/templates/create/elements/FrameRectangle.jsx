  ...
  return (
    ...
    >
      {({ isEditing }) => (
        <div
          ref={setNodeRef}
          style={{ ...element.style, width: `${element.width}px`, height: `${element.height}px` }}
                  ...
                  <Fragment key={el.id}>
                    {components[el.type] ? (
                      createElement(components[el.type], {
                        ...
                        onChange: (e) => {
                          onChange({ ...element, children: element.children.map((el) => (el.id === el.id ? e : el)) });
                        },
                      })
                    ) : (
...
FrameRectangle.propTypes = elementPropTypes;
