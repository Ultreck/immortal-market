...
const MapAfrica = ({ element, active, highlighted, width, onClick, onChange }) => {
  return (
    <ElementWrapper
      element={element}
      onClick={onClick}
      ...
      fit
    >
      <MapAfricaContent element={element} />
    </ElementWrapper>
  );
};

export const MapAfricaPresent = ({ element }) => {
  return <MapAfricaContent element={element} />;
};

export const MapAfricaPreview = () => {
  return <MapAfricaContent element={{ config: { data: [], fill: '#f9fafb', stroke: '#f9fafb' } }} />;
};

const MapAfricaContent = ({ element }) => {
  const { el, assignColor, renderLabels } = useMapElement(element);

  return (
    <svg width="100%" viewBox="0 0 1227 1010" fill={element.config.fill} xmlns="http://www.w3.org/2000/svg" ref={el}>
      <path
        id="AO"
...
MapAfrica.propTypes = ElementPropTypes;
MapAfricaPresent.propTypes = {
  element: PropTypes.object.isRequired,
};
MapAfricaContent.propTypes = {
  element: PropTypes.object.isRequired,
};

export default MapAfrica;
