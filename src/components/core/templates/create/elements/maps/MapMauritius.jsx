...
const MapMauritius = ({ element, active, highlighted, width, onClick, onChange }) => {
  return (
    <ElementWrapper
      element={element}
      onClick={onClick}
      ...
      fit
    >
      <MapMauritiusContent element={element} />
    </ElementWrapper>
  );
};
...
export const MapMauritiusPreview = () => {
  return <MapMauritiusContent element={{ config: { data: [], fill: '#f9fafb', stroke: '#f9fafb' } }} />;
};

const MapMauritiusContent = ({ element }) => {
  const { el, assignColor, renderLabels } = useMapElement(element);

  return (
    <svg ref={el} width="100%" viewBox="0 0 380 442" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
