...
import { cn, FORMFIELDTYPE } from '@/lib/utils.js';
...
import { useCreateForm, useGetForms } from '@/api/business';
...
export const FormPresent = ({ element }) => {
  ...
  const { data: { forms = [] } = {} } = useGetForms({ business, design, element: element.id });
  ...
  const handleSubmit = async () => {
    try {
      console.log(answers);
      const result = await createForm({ form: answers, element: element.id });
      console.log(result);
...
const FormContent = ({ element, className = '', onChange = () => {}, present = false, handleSubmit = () => {},disabled=false }) => {
  return (
    <div
      style={{
        ...element.style,
