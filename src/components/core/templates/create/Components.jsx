...
const Components = () => {
  ...
  const separateElements = () => {
    ...
    elements.forEach(element => {
      if (element.id === 'table' || element.id === 'key-value') {
        data.push(element);
      } else {
        design.push(element);
      }
  ...
  const { design, data } = separateElements();
  ...
  const designGroups = groupElements(design);
  const dataGroups = groupElements(data);
  ...
  return (
    <>
      ...
      {tab === 'design' && renderGroups(designGroups)}
      {tab === 'data' && renderGroups(dataGroups)}
    </>
  );
};
