...
const AnalyzeMono = ({ onBack }) => {
  ...
  const { data, isLoading } = useGetMonoInstitutions({
    key: settings?.statement?.monoSecretKey,
  ...
  return (
    <>
      {!selected ? (
        <>
          ...
          {!hasCredentials && (
            <div className="bg-red-500 text-white rounded-2xl px-8 py-4 mb-6 flex items-start space-x-4">
              <p>Your mono credentials has not been configured, please setup your credentials to proceed</p>
        ...
        <AnalyzeMonoLogin institution={selected} onBack={() => setSelected(null)} />
      )}
    </>
  );
};
...
const AnalyzeMonoLogin = ({ institution, onBack }) => {
  ...
  const { business } = useBusiness();
  const { data: { settings } = {} } = useGetBankingSettings(business._id);
  const monoSecretKey = settings.statement.monoSecretKey;
  const monoApp = settings.statement.monoApp;
  const { mutateAsync: createSession, isPending: isCreateSessionLoading } = useCreateMonoSession({
    key: monoSecretKey,
  });
  const { mutateAsync: login, isPending: isLoginLoading } = useLoginMono({ key: monoSecretKey });
  const { mutateAsync: commitSession, isPending: isCommitSessionLoading } = useCommitMonoSession({
    key: monoSecretKey,
  });
  const { mutateAsync: getTransactions } = useGetMonoTransactions({ key: monoSecretKey });
  ...
  const handleLogin = async (values) => {
    ...
    try {
      const {
        ...
        auth_method: institution.auth_methods?.[tabIndex]?.type,
      });
  ...
  const handleGetTransactions = async (code, n = 10) => {
    ...
    if (n === 0) {
      setError(
        'We could not find any transactions in this account, please contact support if you think this is a mistake'
      );
        ...
        <>
          {view === 'form' && (
            <>
              ...
              <div>
                ...
                {institution.auth_methods.length > 0 ? (
                  <Tabs
                    index={tabIndex}
                    onChange={setTabIndex}
                    className="mt-8"
                    tabs={institution.auth_methods.map((method) => ({
                      key: method._id,
                      name: method.name,
                      panel: (
                        <>
                          ...
                          <form onSubmit={handleSubmit(handleLogin)}>
                            ...
                            <Button type="submit" className="mt-8" loading={isCreateSessionLoading || isLoginLoading}>
                              Login
                            </Button>
                          </form>
                        </>
                  ...
                  <>
                    <h4 className="mb-6 mt-6 text-lg font-medium">{institution.ui.title}</h4>
                    <form onSubmit={handleSubmit(handleLogin)}>
                      {!!error && <div className="px-6 py-3 rounded-2xl bg-red-500 text-white mb-6">{error}</div>}
                      <div className="space-y-3">
                        <Input
          ...
          {view === 'accounts' && (
            <>
              ...
              <img src={institution.icon} alt={institution.name} className="w-24 mb-6" />
              <div>
                <h4 className="mb-6 px-2 text-lg font-medium">Select preferred account below to continue</h4>
                {!!error && <div className="px-6 py-3 rounded-2xl bg-red-500 text-white mb-6">{error}</div>}
                <div className="space-y-3">
