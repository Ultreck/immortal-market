...
const AnalyzeMbs = ({ onBack }) => {
  ...
  return (
    <>
      {view === 'form' && <InitializeForm onTicket={handleFormDone} onBack={onBack} />}
      {view === 'ticket' && <TicketForm data={data} onBack={() => setView('form')} />}
    </>
  );
};
...
const InitializeForm = ({ onTicket, onBack }) => {
  ...
  const {
    ...
    isInitialLoading: isGetAccountNameLoading,
    error: resolveError,
  } = useGetAccountName({
    accountNumber: watch().account,
    bankCode: watch().bank,
  });
