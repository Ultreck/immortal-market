    ...
    <Card>
      ...
      <div className="divide-y divide-gray-300">
        ...
        <div className="px-6 py-2 grid grid-cols-2 gap-6 items-center">
          ...
          <div>
            {highlight.most_frequent_salary_amount.length === 0
              ? 'No salary'
        ...
        <div className="px-6 py-2 grid grid-cols-2 gap-6 items-center">
          ...
          <div>
            {highest_deposit_sweep?.length
              ? `${Math.min(...highest_deposit_sweep)} and ${Math.max(...highest_deposit_sweep)} days`
              : 'N/A'}
          </div>
        </div>
        <div className="px-6 py-2 grid grid-cols-2 gap-6 items-center">
          ...
          <div>
            {highlight.each_deposit_sweep?.location
              ? `${Math.min(...(highlight.each_deposit_sweep || [0]))} and ${Math.max(
                  ...(highlight.each_deposit_sweep || [0])
                )} days`
              : 'N/A'}
          </div>
        </div>
        <div className="px-6 py-2 grid grid-cols-2 gap-6 items-center">
          ...
          <div>{`${highlight.clear_cheque} cleared / ${highlight.bounced_cheque} bounced`}</div>
        </div>
      </div>
    </Card>
  );
