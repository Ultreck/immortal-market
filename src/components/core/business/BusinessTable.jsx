      ...
      {allColumns.map(column => (
        <div key={column.id} className="inline-block mr-4">
          <label>
            ...
            <input type="checkbox" className='form-checkbox h-3 w-3 !text-sky-600' {...column.getToggleHiddenProps()} />{' '}
            {column.Header}
          </label>
        </div>
      ))}
...
const Table = ({ columns, data, updateMyData, onConsoleLog }) => {
  ...
  return (
    <div className="p-4 shadow-md mb-20 rounded-md">
      ...
      <div className='flex gap-2 items-end justify-between flex-wrap'>
        ...
        <button onClick={() => onConsoleLog(visibleColumns, rows)} className="mb-4 p-2 bg-sky-600  text-white rounded-full px-4">
          Log Data
        </button>
      </div>
      <div className="overflow-x-auto scrollbar-hide">
        <table {...getTableProps()} id="table-to-print" className="min-w-full divide-y divide-gray-200">
          ...
          <tbody {...getTableBodyProps()} className="bg-white divide-y divide-gray-200 italic">
            {rows.map((row, i) => {
              prepareRow(row);
              return (
