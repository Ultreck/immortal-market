...
const PreviewData = () => {
  ...
  return (
    <div>
      <Title title={'Preview your data'} sub={'select from multiple sources'} className="mb-8" />
      ...
      >
        <Radio color="default" value="file 1">
          <span className="text-gray-700 dark:text-gray-300/80">File 1</span>
        </Radio>
        <Radio color="default" value="file 2">
          <span className="text-gray-700 dark:text-gray-300/80">File 2</span>
        </Radio>
        <Radio color="default" value="file 3">
          <span className="text-gray-700 dark:text-gray-300/80">File 3</span>
        </Radio>
      </RadioGroup>
      ...
      <div className="flex flex-col gap-7 flex-1 overscroll-auto overflow-auto scrollbar-hide">
        {sampledata?.map((dt) => (
          <div
            key={dt._id}
            className="border dark:border-gray-700  flex justify-between p-6 flex-wrap rounded relative"
          >
            <div
              className={cn(
                'bg-[#fafafa]/90 dark:bg-[#18181b]/90 absolute z-10 top-0 left-0 w-full h-full  items-center justify-center',
                editingIndex === dt?._id ? 'flex' : 'hidden'
              )}
            >
              <div className="flex gap-6">
                <Button onClick={() => unLink(dt?._id)} variant="shadow" color="primary" className="rounded">
                  {dt?.linked ? 'Unlink' : 'Link'}
                </Button>
                <Button onClick={() => setEditingIndex(null)} variant="bordered" color="danger" className="rounded-md">
                  Cancel
                </Button>
              </div>
            </div>
            ...
            <div className="w-20 md:w-32  mt-4 ">
              <div
                className="flex gap-2 border rounded-full justify-end items-center max-w-[4.5rem] px-1 ml-auto shadow-md cursor-pointer"
                onClick={() => setEditingIndex(dt?._id)}
              >
