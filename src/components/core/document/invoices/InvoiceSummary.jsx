          ...
          <>
            ...
            <div className="mt-8 divide-y">
              ...
              <div className="flex items-start justify-between space-x-4 py-2">
                ...
                <div className="text-end">{invoice.date ? format(new Date(invoice.date), 'do MMM, yyyy') : 'N/A'}</div>
              </div>
              <div className="flex items-start justify-between space-x-4 py-2">
                  ...
                  {invoice.items.map((item) => (
                    <div key={item.description} className="py-2">
                      <div className="flex justify-between items-center space-x-6">
                        ...
                        <p>{formatCurrency(item.total, invoice.currency)}</p>
                      </div>
              ...
              <div className="flex items-start justify-between space-x-4 py-2">
                ...
                <div className="text-end">{formatCurrency(invoice.subtotal, invoice.currency)}</div>
              </div>
              <div className="flex items-start justify-between space-x-4 py-2">
                <div className="opacity-70 whitespace-nowrap">Tax</div>
                <div className="text-end">{formatCurrency(invoice.tax, invoice.currency)}</div>
              </div>
              <div className="flex items-start justify-between space-x-4 py-2">
                <div className="opacity-70 whitespace-nowrap">Discount</div>
                <div className="text-end">{formatCurrency(invoice.discount, invoice.currency)}</div>
              </div>
              <div className="flex items-start justify-between space-x-4 py-2">
                <div className="opacity-70 whitespace-nowrap">Shipping</div>
                <div className="text-end">{formatCurrency(invoice.shipping, invoice.currency)}</div>
              </div>
              <div className="flex items-start justify-between space-x-4 py-2">
                <div className="opacity-70 whitespace-nowrap">Total</div>
                <div className="text-end">{formatCurrency(invoice.total, invoice.currency)}</div>
              </div>
            </div>
          </>
        ) : (
          <EditInvoice invoice={invoice} onBack={() => setEdit(false)} />
        )}
      </Card>
    </>
  );
